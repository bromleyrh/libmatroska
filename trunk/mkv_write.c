...
_cvt_string_to_utf8(char **dst, json_value_t src)
{
    ...
    if (json_val_get_type(src) != JSON_STRING_T)
        return -EILSEQ;
                      ...
                      json_value_t src, const char *name, struct ctx *ctx)
{
    ...
    if (json_val_get_type(src) != JSON_NUMBER_T)
        return -EILSEQ;
...
write_mkv(int infd, struct ctx *ctx)
{
    ...
    res = json_parse_text(NULL, 0, &json_read_cb_strip_comments, &rctx, &jval);
    if (res != 0)
        goto err3;
    ...
    for (i = 0; i < m; i++) {
        ...
        if (!block) {
            ...
            if (res != 0)
                goto err8;
            if (json_val_get_type(obje.value) != JSON_NUMBER_T) {
                res = -EILSEQ;
                goto err9;
            }
